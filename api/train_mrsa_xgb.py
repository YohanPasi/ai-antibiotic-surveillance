...
def train_xgb():
    print("Starting XGBoost Training (Challenger) — Schema v2...")
    ...
    # growth_time: -1 sentinel for NULL (non-blood samples — not a median imputation)
    X['growth_time'] = X['growth_time'].fillna(-1)
    for col in CATEGORICAL_FEATURES:
        X[col] = X[col].fillna('Unknown')

    # CRITICAL: remainder='drop' — never 'passthrough', which silently corrupts feature order
    ...
    pipeline_path = os.path.join(ARTIFACT_DIR, f'mrsa_xgb_pipeline_{MODEL_VERSION}.pkl')
    joblib.dump(pipeline, pipeline_path)
    print(f"Saved pipeline to {pipeline_path}")

    # Feature order lock — all 3 scripts write the same file to guarantee consistency
    ...
    metrics_path = os.path.join(ARTIFACT_DIR, f'xgb_training_report_{MODEL_VERSION}.json')
