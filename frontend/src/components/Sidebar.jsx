...
const Sidebar = ({ activeView, setActiveView, logout }) => {
    ...
    return (
        <aside className="w-60 flex flex-col h-screen sticky top-0 flex-shrink-0 overflow-y-auto
                          bg-gray-950 border-r border-white/5 text-white
            ...
            <div className="px-5 py-5 border-b border-white/5 flex items-center gap-3 flex-shrink-0">
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600
                    ...
                    <span className="text-base font-extrabold text-white leading-none">S</span>
                </div>
                <div>
                    <h1 className="font-extrabold text-sm tracking-tight text-white leading-none">Sentinel</h1>
                    <p className="text-[10px] text-gray-500 uppercase tracking-[0.15em] mt-0.5">AMR Surveillance</p>
                </div>
            </div>
                        ...
                        <div key={gIdx} className="mb-1">
                            ...
                            >
                                <div className="text-left">
                                    ...
                                    {group.subtitle && (
                                        <p className="text-[10px] text-gray-600 mt-0.5">{group.subtitle}</p>
                                    )}
                                </div>
                                {isExpanded
                            ...
                            <AnimatePresence initial={false}>
                                {isExpanded && (
                                    ...
                                    >
                                        <div className="mt-0.5 space-y-0.5 pl-2 border-l border-white/5 ml-3">
                                            {group.items.map(item => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${isActive ? 'text-emerald-400' : 'text-gray-600'}`} />
                                                        {item.label}
                                                        {isActive && (
